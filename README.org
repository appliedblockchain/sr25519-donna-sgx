...
| secret                     | the private key of the keypair to sign the message, 64 bytes long |
...
| param         | description                                                          |
| public_key    | the corresponding public key that signing the message, 32 bytes long |
| private_key   | the private key of the keypair to sign the message, 64 bytes long    |
#+BEGIN_SRC C
void sr25519_public_key_from_private_key(sr25519_public_key public_key, sr25519_secret_key private_key)
#+END_SRC
[[https://github.com/appliedblockchain/sr25519-donna-sgx/blob/master/example/src/main.c#L233][example]]
* Author
